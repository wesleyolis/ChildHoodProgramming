...
public class Project7
{
	...
	c = new Console ();
	String Resof = "", DisplayRes = "";
	int loop2 = 0, loop_times, disamount = 0;
	...
	for (;;) //initializing programe loop and heading loop 'Start'
	{
		...
		else
		{
		    ...
		    c.drawString (DisplayRes, 290, 70);
			...
			for (loop2 = codeback.length () - 1 ; loop2 > -1 ; loop2--)
			{
			    ...
			    if (codeback.charAt (loop2) == ':' & Dot == Display + 1)
			    {
			    ...
			    {
				if (codeback.charAt (loop2) == ':' & Dot == Display)
				{
