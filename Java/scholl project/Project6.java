...
public class Project6
{
	...
	for (;;) //initializing programe loop and heading loop 'Start'
	{
		...
		for (;;)
		{
		    ...
		    Finsh = 0;
		    Dot = 0;
		    for (int StFin = 0 ; StFin < codeback.Length () ; DtFin++)
		    {
			if (codeback.chatAt (loop) = ':')
			{
			    Dot++;
		    ...
		    if (option_main != 5)
		    {
			for (loop = 0 ; loop < codeback.length () ; loop++)
			{
			    ...
			    Resnum = (num_char2 * (Math.pow (baseI, loop)));
			    Resaultnum = (Resaultnum + Resnum);
			}
		    }
		    else
		    {
			Resaultnum = (double) codenum;
		    }
		    c.println (Resaultnum);
