	...
	else
	{
		...
		*DFA_Constructed = 1;						/*Say DFA is constructed, but change it on error*/
	    ...
	    if(*DFA_Constructed  == 1)		/*If no Errors yet Contine error Cheacking, Error Teminating*/
	    {
		    ...
		    if((Row < N) && (Colom < N))			/*Test to see if there leas than the required pixels*/
		    {
...
void menu()
{
	...
	int *DFA = malloc(DFA_Size*4*sizeof(int));		/*initialize the DFA Array,using malloc so can have image with more 8 States*/
	...
	while(option !='0')
	{
		...
		{
			case '1':
				ReadImage_CreateDFA(DFA,&Highest_DFA_Row,N,DFA_Size,&DFA_Constructed);
			break;

			case '2':
				if(DFA_Constructed == 1)
				{
				Display_DFA(DFA,Highest_DFA_Row);
				}
			break;
