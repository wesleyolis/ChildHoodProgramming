...
struct Token_v			/* token type and its value */
{
	...
	TokenType_c disc;		/*Discription fied to hold addtional information*/
...
extern boolean insert( TokenType_t token, char *name , int size, TokenType_c DISC)
{
	struct Token_v *curr;
	int hashValue;
	...
	if(Hash[hashValue] == NULL)		/*does an entry exist in this section list*/
	{
		...
		if(Hash[hashValue] != NULL)
		{
			...
			if(Hash[hashValue]->ident != NULL)
			{
				...
				Hash[hashValue]->disc = DISC;
				DISP = DISP + size;						/*INcrese the varible displace ment*/
...
extern boolean find( char *name, TokenType_t *token, int *disp, TokenType_c *DISC) 
{
	struct Token_v *curr;
	int hashValue;
