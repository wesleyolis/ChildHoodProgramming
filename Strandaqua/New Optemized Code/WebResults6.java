...
public class WebResults6
{
    ...
    private Vector Athletes = new Vector (300, 25), Clubs = new Vector (30, 3), Events = new Vector (80, 10), Type = new Vector (2, 1);
	...
	HTML_Type = "type = new Array(\"Finals\",\"Prelims\");";
	...
	while (res.next ())
	{
	    ...
	    {
		temp += t2 + "\",\"" + t3 + "\",\"" + t4 + "\",\"" + t5 + "\",\"" + a1 + "\",\"" + a2 + "\",\"" + a3 + "\",\"" + a4 + "\",\"";
	    }
	    else
	    {
		HTML_Relay += t2 + "\",\"" + t3 + "\",\"" + t4 + "\",\"" + t5 + "\",\"" + a1 + "\",\"" + a2 + "\",\"" + a3 + "\",\"" + a4 + "\",\"";
	    }
	    event = t7;
	}
	HTML_Relay += temp;
	...
	DataRequest = DataBase.createStatement ();
	HTML_Loc = "loc = new Array(\"";
	res = DataRequest.executeQuery ("SELECT RESULT.F_P, RESULT.MTEVENT, Count(RESULT.RESULT) AS CountOfRESULT FROM RESULT INNER JOIN MTEVENT ON RESULT.MTEVENT = MTEVENT.MtEvent GROUP BY RESULT.F_P, RESULT.MTEVENT, MTEVENT.Meet, MTEVENT.Session, MTEVENT.MtEv, MTEVENT.MtEvX HAVING (((MTEVENT.Meet)=" + meet + ")) ORDER BY MTEVENT.Session, MTEVENT.MtEv, MTEVENT.MtEvX;");
	String t1, t2;
	...
	while (res.next ())
	{
	    try
	    {
		type = getTypeIndex (res.getString (1));
	    }
	    catch (NullPointerException e)
	    {
		type = 0;
    ...
    {
	//retrives Individual Results
	HTML_Results = "athres = new Array(\"";
	String temp = "";
	Statement DataRequest = null;
	ResultSet res = null;
	DataRequest = DataBase.createStatement ();
	res = DataRequest.executeQuery ("SELECT  RESULT.MTEVENT, RESULT.ATHLETE, RESULT.SCORE, RESULT.PLACE FROM ((RESULT INNER JOIN Athlete ON RESULT.ATHLETE = Athlete.Athlete) INNER JOIN TEAM ON RESULT.TEAM = TEAM.Team) INNER JOIN MTEVENT ON RESULT.MTEVENT = MTEVENT.MtEvent WHERE (((RESULT.I_R)='I') AND ((RESULT.MEET)=" + meet + ")) ORDER BY TEAM.TName, Athlete.Last, Athlete.First, RESULT.F_P;");
	String t2, t3, t4, event = "";
	int t1;
	while (res.next ())
	{
