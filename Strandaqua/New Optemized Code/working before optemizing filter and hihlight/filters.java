...
function fil ()
{
    ...
    parent.h.document.write ("<form name='F'><table border='0' width='640' cellspacing='0' cellpadding='0'><tr><td align='center'><table border='0' width='590' cellspacing='4' cellpadding='0'><tr><td colspan ='2'><b>Session </b><select onchange='parent.page()' name='sess'><option value='-1' selected>All<\/option>");
    ...
    parent.h.document.write ("<\/select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='checkbox' onclick='parent.page()' name='high' value='1'><\/td><td colspan='3' align='left'><b>Clubs&nbsp; <\/b><font size='1'><select size='1' name='Club' onchange='parent.page()' style='font-weight: 700;position:relative; width:300'><option selected>All Clubs<\/option>");
...
function result2 (pos, num, e)
{
	...
	if (parent.h.document.F.around.selectedIndex != 0)
	{
		...
		if (mch [c] == true)
		{
		    ...
		    {
			d += "<tr bgColor='#6699FF'><td><b>" + parent.res [p] + "</b></td><td><A href='javascript:parent.disath(" + parent.res [p + 1] + ");'><b>" + (parent.ath [parent.res [p + 1]]) + "</b></a></td><td><b>" + (parent.ath [(parent.res [p + 1] * 1) + 1]) + "</b></td><td><b>" + (parent.clubs [(parent.ath [(parent.res [p + 1] * 1) + 2] * 1) + 1]) + "</b></td><td><b>" + (parent.ath [(parent.res [p + 1] * 1) + 3]) + "</b></td><td><b>" + (parent.ath [(parent.res [p + 1] * 1) + 4]) + "</b></td><td><b>" + parent.res [p + 2] + "</b></td></tr>";
		    }
		}
	    }
	}
...
function athres ()
{
    ...
    //sorts the data by stroke then distance then type ,final,prelim...

    /*for (a = 0 ; a < ares.length ; a++)
    {
	for (s = 1 ; s < ares [a] [0] ; s += 3)
	{
	    for (s2 = s + 3 ; s2 < ares [a] [0] + 1 ; s2 += 3)
	    {
		st = (parent.events [ares [a] [s] + 5] - parent.events [ares [a] [s2] + 5]);
		if (st > 0 || (st == 0 & (parent.events [ares [a] [s] + 4] - parent.events [ares [a] [s2] + 4]) > 0))
		{
		    for (sw = 0 ; sw < 3 ; sw++)
		    {
			// alert ("switch");
			t = ares [a] [s2 + sw];
			ares [a] [s2 + sw] = ares [a] [s + sw];
			ares [a] [s + sw] = t;
		    }
		}
	    }
...
function athressort (a)
{
	...
	{
	    st = (parent.events [ares [a] [s] + 5] - parent.events [ares [a] [s2] + 5]);
	    st2 = (parent.events [ares [a] [s] + 4] - parent.events [ares [a] [s2] + 4]);
...
function disjump (j)
{
    parent.m.document.location.href = "#e" + j;
    Winclose ();
    parent.m.focus ();
}
...
{
    LoadWin = null;
    loadWin (10, 10, 440, 520, "Swimmer Results", true);
    //+<tr><td  width='40' style='border-left-style: solid; border-left-width: 1px; border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px'><b>Event</b></td><td  width='40' style='border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px'><b>Type</b></td><td  width='55' style='border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px'><b>Gender</b></td><td  width='81' style='border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px'><b>Age</b></td><td  width='60' style='border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px'><b>Distance</b></td><td  width='85' style='border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px'><b>Stroke</b></td><td  width='40' style='border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px'><b>Place</b></td><td style='border-right-style: solid; border-right-width: 1px; border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px' ><b>Time</b></td></tr>
    LoadWin.document.write ("<table border='0' width='480' cellspacing='0' cellpadding='0'><tr><td><b>" + parent.ath [ath] + ", " + parent.ath [ath + 1] + "<br>Gender: " + gen (parent.ath [ath + 3]) + "&nbsp;&nbsp; Age: " + parent.ath [ath + 4] + "<br>Club: " + parent.clubs [parent.ath [ath + 2]] + "</b></td><td width='94' align='right' valign='top'>&nbsp;</td><td width='78' align='right' valign='top'><b><font size='4'><a href='javascript:parent.focus();parent.window.print(2)'>Print</a></font></b><br>&nbsp;</td></tr></table><br><table border='0' width='400' cellspacing='0'>");
    athressort (ath);
    strk = 0;
	...
	if (strk != parent.events [ares [ath] [q] + 5])
	{
	    ...
	    LoadWin.document.write ("<tr><td></td><td colspan = '2' align='left'><b>" + stk (parent.events [ares [ath] [q] + 5]) + "<b></td><td colspan='4'><Font Size='2'><b>pos</b></Font></td></tr>");
	}
	if (evnt [(ares [ath] [q] / 7)] == true)
	{
	    LoadWin.document.write ("<tr><td width ='50px'><A href=\"javascript:opener.disjump(" + ares [ath] [q] + ");\" ><b>" + parent.events [ares [ath] [q] + 1] + "</b></A></td><td width ='50px'>" + parent.events [ares [ath] [q] + 4] + "m</td><td width ='60px'>" + parent.type [ares [ath] [q + 2]] + "</td><td width='70px'>" + age (parent.events [ares [ath] [q] + 3]) + "</td><td width ='30px'>" + parent.res [ares [ath] [q + 1]] + "</td><td width ='70px'>" + parent.res [ares [ath] [q + 1] + 2] + "</td><td>" + gen (parent.events [ares [ath] [q] + 2]) + "</td></tr>");
	}
	else
	{
    ...
    //javascript:disjump(" + ares [ath] [q] + ");parent.m.document.location.href='#e" + ares [ath] [q] + "';
