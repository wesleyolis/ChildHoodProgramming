...
public class WebResults7
{
    ...
    private Vector Athletes = new Vector (300, 25), Clubs = new Vector (30, 3), Events = new Vector (80, 10), Type = new Vector (2, 1), Results = new Vector (300, 100), diff = new Vector (50, 20);
    ;
	...
	try
	{
	    ...
	    // System.out.println (HTML_Relay);
	    // getResLocation ("324");
	    // System.out.println (HTML_Loc);
	    // System.out.println (HTML_Type);
	...
	ResultSet res = null;
	DataRequest = DataBase.createStatement ();
	res = DataRequest.executeQuery ("SELECT MTEVENT.Session, MTEVENT.MtEv, MTEVENT.MtEvX, MTEVENT.Sex, MTEVENT.Lo_Hi, MTEVENT.Distance, MTEVENT.Stroke, MTEVENT.I_R, MTEVENT.MtEvent FROM MTEVENT WHERE (((MTEVENT.Meet)=" + meet + ")) ORDER BY MTEVENT.Session, MTEVENT.MtEv, MTEVENT.MtEvX;");
    ...
    void getExceptions (String meet) throws SQLException
    {
	diff.removeAllElements ();
	Statement DataRequest = null;
	ResultSet res = null;
	DataRequest = DataBase.createStatement ();
	res = DataRequest.executeQuery ("SELECT Count(RESULT.ATHLETE) AS CountOfATHLETE, RESULT.MTEVENT, RESULT.F_P FROM RESULT INNER JOIN MTEVENT ON RESULT.MTEVENT = MTEVENT.MtEvent GROUP BY RESULT.MTEVENT, RESULT.F_P, RESULT.I_R, RESULT.ATHLETE, MTEVENT.Meet HAVING (((RESULT.I_R)='I' Or (RESULT.I_R)='R') AND ((RESULT.ATHLETE)=0) AND ((MTEVENT.Meet)=" + meet + ")) ORDER BY RESULT.MTEVENT, RESULT.F_P;");
	while (res.next ())
	{
	    diff.addElement (res.getString (2) + res.getString (3));
	}
	diff.trimToSize ();
	res.close ();
	DataRequest.close ();
    ...
    boolean getExceptionMatch (String T)
    {
	int i = 0;
	for (; i < diff.size () ; i++)
	{
	    if (String.valueOf (diff.elementAt (i)).compareTo (T) == 0)
	    {
		return true;
	    }
	}
    ...
    void getResLocation (String meet) throws SQLException
    {
	...
	while (res.next ())
	{
		...
		t1 = res.getString (3);
		if (getExceptionMatch (t1 + t4))
		{
		    t3 = (Integer.parseInt (res.getString (4)) - 1);
		}
		else
		{
		    t3 = Integer.parseInt (res.getString (4));
		    //while (eventnum < getEventsIndex (t1))
		    //{
		    //    eventnum++;
		    //}
		}
    ...
    void getAthRes (String meet) throws SQLException
    {
	//ath results order by stroke then distance then type (Final,Prelim)
	Statement DataRequest = null;
	ResultSet res = null;
	DataRequest = DataBase.createStatement ();
	res = DataRequest.executeQuery ("SELECT RESULT.ATHLETE, RESULT.RESULT FROM ((RESULT INNER JOIN Athlete ON RESULT.ATHLETE = Athlete.Athlete) INNER JOIN TEAM ON RESULT.TEAM = TEAM.Team) INNER JOIN MTEVENT ON RESULT.MTEVENT = MTEVENT.MtEvent WHERE (((RESULT.MEET)=" + meet + ")) ORDER BY RESULT.ATHLETE, RESULT.STROKE, RESULT.DISTANCE;");
	res.next ();
	String t1 = res.getString (1), t2, temp = "{" + getResultsIndex (res.getString (2)) + ",";
	String ath = t1;
	while (res.next ())
	{
	...
	{
	    DataRequest = DataBase.createStatement ();
	    meetsres = DataRequest.executeQuery ("SELECT MEET.Meet, MEET.MName, MEET.Course, MEET.Start, MEET.End, MEET.Location, MEET.Meet FROM MEET WHERE (((Meet.Start)<Date())And ((Meet.Start)>(Date()-140))) ORDER BY MEET.Start DESC;");
	}
	...
	try
	{
		...
		if (l.indexOf (resData [1].trim ()) == -1)
		{
			...
			file meetres = new file ("Meets", MName.replace (' ', '_') + "_" + resData [3].substring (0, 10) + ".htm");
